...
class DealerController extends Controller
{
    ...
    public function dashboard(Request $request): View
    {
        ...
        if ($dealerId) {
            ...
            $leadsRaw = DB::select(
                ...
                WHERE (l."ISDELETED" IS NULL OR l."ISDELETED" = FALSE) AND l."ASSIGNEDTO" = ?
        ...
        if ($dealerId) {
            ...
            switch ($tab) {
                ...
                case 'cancelled': $statusFilter = ['CANCELLED'];
                    break;
                case 'rewarded': $statusFilter = ['REWARDED', 'REWARD', 'PAID'];
                    break;
                case 'pending-payouts': $statusFilter = ['COMPLETED', 'CASE COMPLETED'];
                    break;
                case 'inquiries':
                default:
                    $statusFilter = null;
            ...
            try {
                ...
                if (! empty($leadIds)) {
                    ...
                    foreach ($dealRows as $dr) {
                        $lid = (int) ($dr->LEADID ?? 0);
                        if ($lid > 0) {
                            $dealtMap[$lid] = $dr->DEALTPRODUCT ?? null;
                        }
                    }
                    $rewardedDateMap = [];
    ...
    public function reports(Request $request): View
    {
        ...
        if ($dealerId && $dateFrom && $dateTo) {
            ...
            WHERE (l."ISDELETED" IS NULL OR l."ISDELETED" = FALSE) AND la."USERID" = ? AND la."CREATIONDATE" >= ? AND la."CREATIONDATE" <= ?
