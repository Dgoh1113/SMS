...
Route::middleware(['auth.sms', 'admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::post('/inquiries/mark-failed', [AdminController::class, 'markInquiryCancelled'])->name('inquiries.mark-failed');
    Route::post('/inquiries/check-duplicates-ajax', [AdminController::class, 'checkDuplicatesAjax'])->name('inquiries.check-duplicates-ajax');
...
Route::middleware(['auth.sms', 'dealer'])->prefix('dealer')->name('dealer.')->group(function () {
    ...
    Route::get('/payouts', fn () => redirect()->route('dealer.inquiries', ['tab' => 'pending-payouts']))->name('payouts');
    Route::get('/rewards', fn () => redirect()->route('dealer.inquiries', ['tab' => 'pending-payouts']))->name('rewards');
