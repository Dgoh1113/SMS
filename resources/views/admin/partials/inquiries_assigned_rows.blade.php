...
<tr class="inquiry-row" data-search="{{ strtolower(($r->COMPANYNAME ?? '') . ' ' . ($r->CONTACTNAME ?? '') . ' ' . ($r->LEADID ?? '')) }}">
    ...
    <td class="inquiries-col-action inquiries-action-cell">
        @if(in_array($arawStatus, ['', 'OPEN', 'CREATED', 'PENDING', 'ONGOING'], true))
        ...
        <button type="button" class="inquiries-btn inquiries-btn-assign inquiries-btn-assign-danger inquiries-mark-failed-btn" data-lead-id="{{ $r->LEADID }}" data-status="{{ $arawStatusDisp }}" title="Mark As Cancelled" aria-label="Mark As Cancelled"><i class="bi bi-flag-fill" aria-hidden="true"></i></button>
    </td>
</tr>
@empty
